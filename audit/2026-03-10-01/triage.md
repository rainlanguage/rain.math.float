...
| A06-20 | 4 | LOW | `pow` function directly uses implementation internals, breaking packed API abstraction | PENDING |
| A07-01 | 4 | LOW | 7 unused imports in `LibDecimalFloatDeploy.sol` | PENDING |
| A08-1/p3 | 3 | LOW | `toDecimalString` missing `@param scientific` documentation | PENDING |
| A08-5/p4 | 4 | LOW | Redundant import path `../../lib/implementation/` in `LibFormatDecimalFloat.sol` | PENDING |
| A08-6 | 4 | LOW | `countSigFigs` is dead production code in `LibFormatDecimalFloat.sol` | PENDING |
| A09-1/p1 | 1 | LOW | `unabsUnsignedMulOrDivLossy` missing exponent overflow check on `exponent + 1` (dupes: A09-15/p2 LOW, A09-20/p5 INFO) | PENDING |
| A09-1/p3 | 3 | LOW | `div` missing all `@param` and `@return` NatSpec tags in implementation | PENDING |
| A09-5 | 3 | LOW | `inv` missing `@param` and `@return` NatSpec tags in implementation | PENDING |
| A09-9 | 3 | LOW | `compareRescale` missing `@param` and `@return` tags in implementation | PENDING |
| A09-11 | 2 | LOW | Six internal functions have no direct test coverage | PENDING |
| A09-12 | 2 | LOW | `minus` ExponentOverflow error path untested | PENDING |
| A09-13 | 2 | LOW | `log10` error paths (`Log10Zero`, `Log10Negative`) completely untested (dupes: A02-1/p2 LOW, A02-2/p2 LOW) | PENDING |
| A09-14 | 2 | LOW | `MulDivOverflow` error path in `mulDiv` untested (dupe: A02-3/p2 LOW) | PENDING |
| A09-16/p4 | 4 | LOW | Magic number `100` (alt small log table byte size) in `lookupAntilogTableY1Y2` | PENDING |
| A09-17/p4 | 4 | LOW | Magic number `2000` (antilog table byte size) in `lookupAntilogTableY1Y2` | PENDING |
| A09-22 | 5 | LOW | `div` scale selection binary search has gaps causing extra while-loop iterations | PENDING |
| A10-1/p1 | 1 | LOW | Unchecked `exponent += eValue` can silently wrap on int256 overflow in `parseDecimalFloatInline` | PENDING |
| A10-1/p4 | 4 | LOW | Split imports from same module in `LibParseDecimalFloat.sol` | PENDING |
| A10-3 | 5 | LOW | Undocumented 67-digit limit and conflated overflow checks in parse rescaling | PENDING |
| A10-7 | 2 | LOW | No dedicated unit test for `ParseDecimalFloatExcessCharacters` from wrapper | PENDING |
| A10-8 | 2 | LOW | No dedicated unit test for `ParseDecimalPrecisionLoss` from `packLossy` in wrapper | PENDING |
