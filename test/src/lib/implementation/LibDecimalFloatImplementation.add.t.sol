...
contract LibDecimalFloatImplementationAddTest is Test {
    ...
    function testAddOneOnePreNormalized() external pure {
        (int256 signedCoefficient, int256 exponent) = LibDecimalFloatImplementation.add(1e37, -37, 1e37, -37);
        assertEq(signedCoefficient, 2e37);
        assertEq(exponent, -37);
    }
    ...
    function testAddingSmallToLargeReturnsLargeExamples() external pure {
        ...
        (int256 signedCoefficient, int256 exponent) = LibDecimalFloatImplementation.add(1e37, 0, 1e37, -37);
        assertEq(signedCoefficient, 10000000000000000000000000000000000001e37);
        assertEq(exponent, -37);
        // Show baseline with reversed order.
        (signedCoefficient, exponent) = LibDecimalFloatImplementation.add(1e37, -37, 1e37, 0);
        assertEq(signedCoefficient, 10000000000000000000000000000000000001e37);
        assertEq(exponent, -37);

        // Show full precision loss.
        (signedCoefficient, exponent) = LibDecimalFloatImplementation.add(1e37, 0, 1e37, -38);
        assertEq(signedCoefficient, 1e37);
        assertEq(exponent, 0);
        // Show same thing again with reversed order.
        (signedCoefficient, exponent) = LibDecimalFloatImplementation.add(1e37, -38, 1e37, 0);
        assertEq(signedCoefficient, 1e37);
        assertEq(exponent, 0);
        ...
        (signedCoefficient, exponent) = LibDecimalFloatImplementation.add(1e37, -20, 1e37, -57);
        assertEq(signedCoefficient, 10000000000000000000000000000000000001e37);
        assertEq(exponent, -57);
        // Reverse order.
        (signedCoefficient, exponent) = LibDecimalFloatImplementation.add(1e37, -57, 1e37, -20);
        assertEq(signedCoefficient, 10000000000000000000000000000000000001e37);
        assertEq(exponent, -57);

        // Only the difference in exponents matters.
        (signedCoefficient, exponent) = LibDecimalFloatImplementation.add(1e37, -20, 1e37, -58);
        assertEq(signedCoefficient, 1e37);
        assertEq(exponent, -20);
        // Reverse order.
        (signedCoefficient, exponent) = LibDecimalFloatImplementation.add(1e37, -58, 1e37, -20);
        assertEq(signedCoefficient, 1e37);
