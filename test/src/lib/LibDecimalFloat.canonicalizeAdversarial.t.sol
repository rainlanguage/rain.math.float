...
/// Adversarial tests for `canonicalize` that deliberately leave the int128/uint8
/// "easy middle" the existing suite stays inside. These target the int224 and
/// int32 type boundaries and, crucially, assert the SAFETY property the existing
/// tests never check: numerically DISTINCT Floats must canonicalize to
/// byte-UNEQUAL results (no collision). They also pin the exponent to the
/// int32.min floor where the scaling loop is capped before the coefficient is
/// maximised, and the int224.min/int224.max coefficient boundaries.
contract LibDecimalFloatCanonicalizeAdversarialTest is Test {
    ...
    /// for small coefficients, the regime the existing tests never reach.
    ...
    /// This is the regime the existing fuzz (uniform int32 exponent) almost
    /// never samples.
    ...
    /// canonicalize (both directions in one test). The existing eq->byte-equal
    /// fuzz only ever feeds equal values; this also feeds unequal ones.
